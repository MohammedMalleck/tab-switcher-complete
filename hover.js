export function handleTableSwitcher(parentClass,childClass){
  ...
  function handleWidths(){
    document.querySelectorAll(`.${childClass}`).forEach((tabEl,index)=>{
      const tabElWidth = tabEl.getBoundingClientRect().width.toFixed(2);
      const numberstowords = getNumbersToWords();
      const propertyName = `--tab-${numberstowords.toInternationalWords(index + 1)}-width`;
      document.querySelector(`.${parentClass}`).style.setProperty(`${propertyName}`,`${tabElWidth}px`);
    });
  };

  //this is a function from a library 
  //here is the link to it  : https://github.com/rajch/numberstowords
  function getNumbersToWords(){
